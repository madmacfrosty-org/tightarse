...
import { applyRuleSets, decide } from "../src/application/apply.js";
...
describe("applying over a range", () => {
  it("writes a categorisation for what the rules place", async () => {
    ...
    const report = await applyRuleSets(deps, "frost", { range: RANGE, now: NOW });
  ...
  it("writes nothing the second time, because idempotency is load-bearing", async () => {
    ...
    const report1 = await applyRuleSets(first.deps, "frost", { range: RANGE, now: NOW });
    ...
    const report2 = await applyRuleSets(second.deps, "frost", { range: RANGE, now: NOW });
  ...
  it("matches on the provider category a transaction carries", async () => {
    ...
    await applyRuleSets(deps, "frost", { range: RANGE, now: NOW });
  ...
  it("decides everything and writes nothing on a dry run", async () => {
    const { deps, written } = ledger([txRow("d1", "SOMEMART SUPERSTORE")]);
    const report = await applyRuleSets(deps, "frost", { range: RANGE, now: NOW, dryRun: true });
