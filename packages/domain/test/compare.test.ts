...
import {
  ...
  type Row,
  scanAll,
} from "../src/application/compare.js";
...
describe("telling one kind of row from another", () => {
  it.each([
    ...
    [
      "categorisation",
      { pk: "T#frost#TX", sk: "2026-03-15T00:00:00Z#CAT#n:abc#built-in" },
    ],
  ...
  it("separates a transaction from its categorisation, which share a partition", () => {
    ...
    const categorisation = {
      pk: "T#frost#TX",
      sk: "2026-03-15T00:00:00Z#CAT#n:abc#built-in",
    };
...
describe("finding real differences", () => {
  it("reports an attribute whose value changed", () => {
    ...
    expect(report.differing[0]).toMatchObject({
      attribute: "amount",
      left: -1299,
      right: 1299,
    });
  ...
  it("reports an attribute the replay added, not just ones it changed", () => {
    ...
    expect(report.differing.map((d) => d.attribute)).toContain(
      "runningBalance",
    );
  });
...
describe("the report", () => {
  ...
  it("names the row kinds it skipped rather than dropping them silently", () => {
    ...
    const text = formatReport(
      compareRows([txn(), categorisation], [txn(), categorisation]),
    );
  ...
  it("truncates a long list rather than printing thousands of lines", () => {
    const many = Array.from({ length: 50 }, (_, i) =>
      txn({ sk: `2026-03-15T00:00:00Z#TX#n:${i}` }),
    );
...
describe("rows it does not recognise", () => {
  ...
  it("omits the skipped section entirely when nothing was skipped", () => {
    ...
    expect(formatReport(compareRows([txn()], [txn()]))).not.toContain(
      "skipped",
    );
  });
});
...
describe("differences that only say when a row was written", () => {
  ...
  it("counts and names what it ignored rather than hiding it", () => {
    ...
    expect(report.ignoredByAttribute).toEqual({
      ingestedAt: 1,
      expiresAt: 1,
      lastSyncedAt: 1,
    });
  ...
  it("summarises differences by attribute, because the distribution is the diagnosis", () => {
    ...
    expect(compareRows(rows, changed).differingByAttribute).toEqual({
      amount: 5,
      currency: 5,
    });
  });
...
describe("reading the whole table", () => {
  it("asks the port for everything and hands it back unchanged", async () => {
    // Covered here now because the test that used to exercise it, reconcile-job,
    // moved to the schedule adapter with the handler it belongs to. Pagination
    // is the adapter's problem: a caller of a port should not have to know that
    // a scan arrives a megabyte at a time.
    const rows = [
      { pk: "T#x", sk: "TX#1" },
      { pk: "T#x", sk: "CAT#1" },
    ];
    const port = { scanAll: async () => rows };

    expect(await scanAll(port)).toEqual(rows);
  });
});

describe("what a difference report may print", () => {
  it("summarises values by type and length, never by content", () => {
    // The rows being compared are a household's real transactions. A report
    // that printed values would put a merchant or a person's name into a log,
    // so every case has to be covered: absent, a string reduced to its length,
    // and anything else rendered structurally.
    const report = {
      onlyInLeft: [],
      onlyInRight: [],
      identical: 0,
      ...
      differingByAttribute: { description: 1, amount: 1 },
      ignoredByAttribute: {},
      comparedByKind: {},
      skippedByKind: {},
    };
    ...
    expect(out).toContain("string(13)");
    expect(out).toContain("absent");
    expect(out).toContain("-1299");
    expect(out).not.toContain("SOME MERCHANT");
  });
});
