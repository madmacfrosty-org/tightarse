 ...
 *
 * Nothing is protected from re-application. `authored` means a rule SET is
 * never regenerated by something derived — it is a statement about the rules,
 * not about what they produce. A categorisation from a hand-written rule is
 * still derived: re-running that rule reproduces it exactly, and freezing it
 * would mean improving the rule never reaches the transactions it already
 * matched, which is precisely what re-application exists to do.
 *
 * There is nothing left for it to protect either, now overrides are rules. A
 * correction is a `transaction` matcher in an authored set, so it survives
 * re-application by being re-derived rather than by being fenced off.
 *
