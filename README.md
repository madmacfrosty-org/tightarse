...
> `@tightarse/fixtures` — see [CONTRIBUTING](CONTRIBUTING.md). A `gitleaks`
...
- **Personal categorisation rules are data, not code.** The rules shipped in
  `agents/categoriser` are national chains that apply to anyone. A household's
...
TABLE=$LEDGER_TABLE npm run rules -w @tightarse/categoriser -- test "SOME DESCRIPTION"

# categorise by hand; the schedule does this daily, rules only
TABLE=$LEDGER_TABLE npm run run-batch -w @tightarse/categoriser -- --mode rules --dry-run
```
